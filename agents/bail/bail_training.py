...
def train_upper_envelope(states, returns, state_dim,
	...
	train_states = torch.cat((train_states.squeeze(), highestS.squeeze().unsqueeze(0)))
...
def plot_envelope(upper_envelope, states, returns, setting, hyper_lst, make_title=False):
	...
	upper_envelope_r = torch.stack(upper_envelope_r).cpu().numpy()
	increasing_ue_indices = np.argsort(upper_envelope_r.reshape(1, -1))
	increasing_ue_vals = upper_envelope_r[increasing_ue_indices[0]]
	MC_r = MC_r[increasing_ue_indices[0]]

	all_ue_loss = np.clip(increasing_ue_vals-MC_r, 0, None).sum() + \
				  np.clip(MC_r-increasing_ue_vals, 0, None).sum()*k_val
	...
	plt.scatter(plot_s, list(MC_r.reshape(1, -1)[0]), s=0.5, color='orange', label='MC Returns')
	plt.plot(plot_s, list(increasing_ue_vals.reshape(1, -1)[0]), color='blue', label="Upper Envelope")
	...
	plt.tight_layout()
	plt.savefig('./agents/bail/plots/' + "ue_visual_%s.png" % setting)
	plt.close('all')
...
def plot_envelope_with_clipping(upper_envelope, states, returns, setting, seed, hyper_lst, make_title=False, S=10000):
	...
	clip_info = '_clip_%.2f_loss_%.2fe6' % (Clipping_value.item(), Clipping_loss.item()/1e6)
	if make_title:
