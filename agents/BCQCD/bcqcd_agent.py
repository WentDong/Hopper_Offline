...
class VAE(nn.Module):
	...
	def reparameterize(self, mu, std):
		...
		return z
	def forward(self, states, inv_countdowns, actions):
		z = self.encoder(torch.cat([states, inv_countdowns, actions], 1))
		mu, lg_std = torch.split(z, z.size(1) // 2, dim=1)
		std = torch.exp(torch.clamp(lg_std, min = -4, max = 15))
		z = self.reparameterize(mu, std)
		return self.decoder(torch.cat([states, inv_countdowns, z], 1)), mu, std

	def call_samples(self, states, inv_countdowns):
		z = torch.randn((states.shape[0], self.latent_dim)).to(self.device).clamp(-0.5, 0.5)
		# print(z.shape, states.shape)
		return self.decoder(torch.cat([states, inv_countdowns, z], 1))
...
class Actor_disturb(nn.Module):
	...
	def forward(self, states, inv_countdowns, actions):
		return torch.clamp(self.phi * self.actor(torch.cat([states, inv_countdowns, actions], 1)) + actions, -1, 1)
...
class Critic(nn.Module):
	...
	def forward(self, states, inv_countdowns, actions):
		return self.Q1(torch.cat([states, inv_countdowns, actions], 1)), self.Q2(torch.cat([states, inv_countdowns, actions], 1))
	def call_single(self, states, inv_countdowns, actions):
		return self.Q1(torch.cat([states, inv_countdowns, actions], 1))
	
class BCQCD(nn.Module):
	def __init__(self, rollout=1000, gamma=0.99, state_dim=11, action_dim=3, latent_dim=10, hidden_dim_VAE=[750,750], hidden_dim_Q = [400,300], Phi = 0.05, lambd = 0.7, tau = 0.005,device = 'cpu', lr=1e-3, lr_critic=None):
		...
		self.VAE_net = VAE(state_dim + 1, action_dim, hidden_dim_VAE, latent_dim, device)
		self.Actor_disturb_net = Actor_disturb(state_dim + 1, action_dim, hidden_dim_Q, Phi, device)
		self.Critic_net = Critic(state_dim + 1, action_dim, hidden_dim_Q, device)
	...
	def reset(self, rollout=None):
		self.countdown = rollout if rollout is not None else self.rollout

	def forward(self, states, countdowns=None):
	...
	def train(self, states, actions, next_states, rewards, not_dones, selects=None, countdowns=None):
		if countdowns is None:
			countdowns = torch.full((states.shape[0], 1), self.countdown, device=self.device).float()
			self.countdown -= 1
		countdowns /= 1000
		...
		if selects is not None:
			VAE_loss = VAE_loss[selects.bool()]
